...
import state, commands, icons, chord
...
proc filelistMessage(element: ptr Element, message: Message,
  ...
  if message == msgPaint:
    ...
    # Chord-leader takes precedence over everything else in this widget.
    # When active, the next keystroke is the follower; we exit chord mode
    # whatever the outcome. Esc cancels without dispatching anything.
    if chord.active:
      ...
      if k.text != nil and k.textBytes > 0:
        let c = k.text[0]
        let consumed = chord.handleFollower(c)
        if not consumed: chord.exit()
        elementRepaint(element, nil)
        return 1
      # any other non-printable just exits chord, doesn't pass through
      chord.exit()
      elementRepaint(element, nil)
      return 1

    # `c` (bare, no modifier) is the leader. Enters chord mode; bookmarks
    # pane swaps for the chord-help element on the left.
    ...
    if code == int(KEYCODE_DOWN) or code == int(KEYCODE_LETTER('J')):
      ...
      return 1
    elif k.text != nil and k.textBytes > 0 and k.text[0] == 'b':
      discard runCommand("bookmark.add", @[])
      return 1
    else:
      return 0
    followSelection(fl)
    ...
    if t.selectedIdx != prevSel: state.notify()
    elementRepaint(element, nil)
    return 1
